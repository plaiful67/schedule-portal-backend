...
4. Site rebuild + deploy is driven from the meta repo (`~/peds-gi-prep-system/`): `make sites-flex-sig` builds the sites; the flex-sig sites are pushed manually from inside `flexsig-giready/` and `flexsig86-giready/` (not by `make deploy`). Re-run `giready-live-site-smoke` after deploy.
