...
# templates, dosing.yaml, practice.yaml, or any script in scripts/.
#
...
# Runs the bowel-prep-generator validate.py (lint mode) on changes that
# could affect rendered output. Skip with: git commit --no-verify

set -e

SKILL_DIR="$(git rev-parse --show-toplevel)"
RELEVANT_PATTERNS='^(templates/|data/dosing\.yaml|practice\.yaml|scripts/.*\.py$)'
