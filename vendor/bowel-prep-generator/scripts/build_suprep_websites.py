...
def build_for_repo(repo_dir, location_id, location, practice_cfg, bands_by_id):
    ...
    p = en_dir / "index.html"
    p.write_text(_inject_analytics(en_html, "suprep", location_id, "en", "suprep"), encoding="utf-8")
    written.append(p)
    ...
    p = es_dir / "index.html"
    p.write_text(_inject_analytics(es_html, "suprep", location_id, "es", "suprep"), encoding="utf-8")
    written.append(p)
