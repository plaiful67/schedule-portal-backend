...
def build_for_repo(repo_dir, location_id, location, practice_cfg, bands_by_id):
    ...
    p = en_dir / "index.html"
    p.write_text(_inject_analytics(en_html, "clenpiq-combined", location_id, "en", "clenpiq"), encoding="utf-8")
    written.append(p)
    ...
    p = es_dir / "index.html"
    p.write_text(_inject_analytics(es_html, "clenpiq-combined", location_id, "es", "clenpiq"), encoding="utf-8")
    written.append(p)
